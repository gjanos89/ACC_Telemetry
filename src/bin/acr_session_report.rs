...
fn extract_lap_sectors(
    ...
    for i in (start + 1)..=end.min(gfx.len() - 1) {
        ...
        if current != previous {
            let sector_no = if previous >= 0 { previous as usize + 1 } else { 0 };
            if (1..=3).contains(&sector_no) {
                if let Some(t) = sector_time_sec(&gfx[i]) {
                    sectors[sector_no - 1] = Some(t);
                }
            }
            previous = current;
        }
    }
    if let Some(t) = sector_time_sec(&gfx[end.min(gfx.len() - 1)]) {
        if sectors[0].is_some() && sectors[1].is_some() {
            sectors[2] = Some(t);
        }
    }
    sectors
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let physics = env::args().nth(1).map(PathBuf::from)
        .ok_or("HasznĂˇlat: acr_session_report.exe <physics.rkyv> [output.html]")?;
    ...
    if !gfx_path.exists() {
        return Err(format!("HiĂˇnyzik a graphics sidecar: {}", gfx_path.display()).into());
    }
    ...
    if boundaries.is_empty() {
        return Err("Nem található befejezett kör a graphics telemetryben.".into());
    }
    ...
    for (li, lap) in laps.iter().enumerate() {
        ...
        let sectors = extract_lap_sectors(&gfx, lap.gfx_start, lap.gfx_end);
    ...
    let data = json!({
        ...
        "best_time": if best_sec > 0.0 { fmt_sec(best_sec) } else { "â€”".to_string() },
        "best_ever": best_ever,
    ...
    let template_path = template_candidates
        ...
        .find(|p| p.exists())
        .cloned()
        .ok_or_else(|| "Nem található a frontend template: src/report/template.html".to_string())?;
